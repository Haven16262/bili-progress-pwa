...
> **修订（2026-09-21 11:20，用户验收后）**：庆祝态数字由 **20px 改为 16px**，且**静态终态不再带柔光**（只留深紫描边 + 黑影；柔光/扫光仅庆祝 3 秒与悬停重放时出现）。下文与参考稿里凡写 20px、静态带柔光之处均以本条为准。同时，「20px/700 属 WCAG 大字 → 3:1」的依据在 16px 下不再成立（16px 加粗不是大字）；底色未改，对比度现状与取舍见 `context.md` backlog。
